...
def start_http_server():
    ...
    with socketserver.TCPServer(("0.0.0.0", 26954), handler) as httpd:
        print(f"[*] {timestamp()} : HTTP server running on port 26954")
        httpd.serve_forever()


def main():
    """Main program entry point. Starts HTTP and receiver servers and handle connections."""
    ...
    http_thread.start()
    time.sleep(1)  # Ensure HTTP server gets time to bind
